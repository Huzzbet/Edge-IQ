/* Edge-IQ AFL ML-style ensemble v2.
   Transparent deterministic ensemble until a trained artifact is available.
   Combines Elo, recent form and scoring margin. Designed to be backtested.
*/
...
function result(g){const h=num(g.home_score??g.hscore??g.home_points),a=num(g.away_score??g.ascore??g.away_points);if(h==null||a==null||h===a)return null;return {y:h>a?1:0,margin:h-a}}
function build(games,{initial=1500,k=22,homeAdv=35,formGames=8}={}){
 ...
 return {ratings,stats,params:{initial,k,homeAdv,formGames},gamesUsed:sorted.length};
}
